...
//		myCurveX = _myTimelineController.snapToRaster(myCurveX);
		...
		case MOVE_BOTH:
			double myCurveMove = _myTransportView.viewXToTime(e.getX()) - _myLastTime;
			_myNewLoopStart = _myLoopStart + myCurveMove;
			_myNewLoopEnd = _myLoopEnd + myCurveMove;
			break;
		case MOVE_START:
			double myCurveMoveStart = _myTransportView.viewXToTime(e.getX());
			_myNewLoopStart = _myTrackContext.snapToRaster(myCurveMoveStart);
			_myNewLoopEnd = _myLoopEnd;
			break;
		case MOVE_END:
			double myCurveMoveEnd = _myTransportView.viewXToTime(e.getX());
			_myNewLoopStart = _myLoopStart;
			_myNewLoopEnd = _myTrackContext.snapToRaster(myCurveMoveEnd);
			break;
		default:
			if (e.getX() < _myStartClickX) {
				_myNewLoopStart = _myTrackContext.snapToRaster(_myTransportView.viewXToTime(e.getX()));
				_myNewLoopEnd = _myTrackContext.snapToRaster(_myStartTime);
			} else {
				_myNewLoopStart = _myTrackContext.snapToRaster(_myStartTime);
				_myNewLoopEnd = _myTrackContext.snapToRaster(_myTransportView.viewXToTime(e.getX()));
			}
			break;
		}
