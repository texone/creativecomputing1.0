...
public class TrackData extends TreeSet<ControlPoint>{
	...
	public CCXMLElement toXML(double theStartTime, double theEndTime) {
		CCXMLElement myTrackDataXML = new CCXMLElement(getClass().getSimpleName());
		ArrayList<ControlPoint> myPoints = rangeList(theStartTime, theEndTime);
	
		for (ControlPoint myPoint:myPoints) {
			CCXMLElement myControlPointXML = myPoint.toXML(theStartTime, theEndTime);
			myTrackDataXML.addChild(myControlPointXML);
		}
		return myTrackDataXML;
	}
	
	public void fromXML(CCXMLElement theTrackDataXML) {
		for (CCXMLElement myControlPointXML:theTrackDataXML) {
			ControlPoint myPoint;
			ControlPointType myType = ControlPointType.valueOf(myControlPointXML.attribute(ControlPoint.CONTROL_POINT_TYPE_ATTRIBUTE));
			...
			case LINEAR:
				...
				break;
			case CUBIC:
				myPoint = new CubicControlPoint();
				break;
			case EXPONENTIAL:
				myPoint = new ExponentialControlPoint();
				break;
			case BEZIER:
			...
			myPoint.fromXML(myControlPointXML);
			add(myPoint);
		}
	}
}
