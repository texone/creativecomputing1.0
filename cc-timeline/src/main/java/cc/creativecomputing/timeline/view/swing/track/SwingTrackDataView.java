...
import cc.creativecomputing.timeline.view.swing.SwingConstants;
import cc.creativecomputing.timeline.view.swing.SwingEventCreatePopup;
import cc.creativecomputing.timeline.view.swing.SwingEventPopup;
import cc.creativecomputing.timeline.view.swing.SwingToolChooserPopup;


@SuppressWarnings("serial")
public class SwingTrackDataView extends JPanel implements TrackDataView{
...
//        if(theDrawInterval){
	        double myInterval = TrackView.GRID_INTERVAL / getWidth() * (_myTrackContext.viewTime());
...
//    }
    ...
    private void renderAutomization() {
		...
		while (myCurrentPoint != null) {
			...
			if (myCurrentPoint.getType() == ControlPointType.EXPONENTIAL) {

				if (myCurrentPoint.hasPrevious()) {
					ControlPoint myPrevious = myCurrentPoint.getPrevious();
					double myX = 0.5f * (myCurrentPoint.time() - myPrevious.time()) + myPrevious.time();
					ControlPoint myTensionHandle = new ControlPoint(myX, _myController.trackData().value(myX));
					Point2D myUserHandle = _myController.curveToViewSpace(myTensionHandle);
					g2d.drawOval(
						(int) myUserHandle.getX() - _myPointSize / 2,
						(int) myUserHandle.getY() - _myPointSize / 2,
						_myPointSize, _myPointSize
					);
				}
				// TODO: paint tension handle at the correct position
			}
	...
	public void renderImplementation() {
		...
		if (_myController.trackType() != TrackType.TIME && _myController.trackType() != TrackType.GROUP) {
			renderAutomization();
		}else {
			renderTimedEvent();
		}
	...
	private void drawTimelineInfos(Graphics g) {
		...
		if (myViewX >= 0 && myViewX <= getWidth()) {
			...
			if (_myController.trackType() != TrackType.TIME && _myController.trackType() != TrackType.GROUP) {
				...
				String myValueString = SwingTrackControlView.VALUE_FORMAT.format(myValue);
				g.drawString(myValueString, myViewX + 10, (int) _myController.curveToViewSpace(new ControlPoint(myTime, myValue)).getY());
			}
		}
