...
const float grid = 0.005;	// Grid offset for normal-estimation
varying vec3 norm, eyeVec;	// Normal
...
void phongDir_VS() {
	...
	//lightDir0 = vec3(0,1,0);//normalize(vec3(gl_LightSource[0].position));
...
void main()
{
	...
	vec4 vVertex = gl_ModelViewProjectionMatrix * spherePos; 
	eyeVec = vVertex.xyz;
