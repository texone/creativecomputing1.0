...
public class DBDisplacementMapPointSubdivisionTest extends CCApp {
	
	
	
	@CCControl(name = "tex displacement", min = 0, max = 100)
	private float _cTextureDisplacement = 0;
	...
	private static class DBLinesShader{
		...
		public DBLinesShader(CCGraphics g) {
			_myShader = new CCGLSLShader(
				"gpu/displacement_geometry_points/vertex.glsl", 
				"gpu/displacement_geometry_points/geometry.glsl", 
				"gpu/displacement_geometry_points/fragment.glsl"
			);
			_myShader.geometryInputType(CCGeometryInputType.POINTS);
			_myShader.geometryOutputType(CCGeometryOutputType.POINTS);
...
//					g.color(1f);
		 ...
		 */
		public void start() {
			_myGraphics.texture(7,_myFilterTexture.attachment(0));
			_myShader.start();
			_myShader.uniform1i("displacementTexture", 0);
			_myShader.uniform1i("filterTexture", 7);
			_myShader.uniform1f("strokeWeight",10f);
	...
	private int _myTextureIndex = 0;
	
	private DBLinesShader _myLinesShader;

	public void setup(){
		for(CCTextureData myData:CCTextureIO.newTextureDatas("content/textures/", CCFileFormat.PNG)) {
...
//		_myDisplacementShader.displacementTexture(_myTextures.get(_myTextureIndex));
//		_myDisplacementShader.textureDisplacement(DBLineControls.textureDisplacement);
//		_myDisplacementShader.color(1,0.5f,0);
	}
...
//		g.image(_myRenderTexture, -_myRenderTexture.width()/2, -_myRenderTexture.height()/2, _myRenderTexture.width(), _myRenderTexture.height());
		...
		g.texture(0,_myTextures.get(_myTextureIndex));
		_myMesh.draw(g);
		g.noTexture();
		_myLinesShader.end();
	}
	...
	public void keyPressed(final CCKeyEvent theEvent) {
		...
		case VK_LEFT:
			_myTextureIndex--;
			if(_myTextureIndex < 0)_myTextureIndex += _myTextures.size();
			break;
		case VK_RIGHT:
			_myTextureIndex++;
			_myTextureIndex %= _myTextures.size();
			break;
		case VK_S:
//			CCScreenCapture.capture("export/deformed_text_"+CCFormatUtil.nf(counter++, 3)+".png", width,height);
	...
	private void createMesh(){
		...
		int _myXResolution = 50;
		
		_myMesh = new CCMesh(CCDrawMode.LINES, _myXResolution * _myYResolution);
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(DBDisplacementMapPointSubdivisionTest.class);
