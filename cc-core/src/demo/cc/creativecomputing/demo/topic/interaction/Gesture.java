...
class Gesture {
	...
	float getPressureFromVelocity(float v) {
		...
		return ((minP + CCMath.max(0, 1.0f - v / scale)) + (damp1 * oldP)) * dampInv;
	}
	...
	void setPressures() {
		...
		float pressure;
		float t = 0;
		float u = 1.0f / (nPoints - 1) * CCMath.TWO_PI;
		for (int i = 0; i < nPoints; i++) {
			pressure = CCMath.sqrt((1.0f - CCMath.cos(t)) * 0.5f);
			path[i].z = pressure;
			t += u;
		}
	}
	...
	void compile() {
		// compute the polygons from the path of CCVector3f's
		if (exists) {
			clearPolys();
			...
			float npm1finv = 1.0f / CCMath.max(1, nPathPoints - 1);
			...
			dx01 = p1.x - p0.x;
			dy01 = p1.y - p0.y;
			hp01 = CCMath.sqrt(dx01 * dx01 + dy01 * dy01);
			if (hp01 == 0) {
				hp02 = 0.0001f;
			}
			...
			for (i = 1; i < nPathPoints; i++) {
				taper = CCMath.pow((lastPolyIndex - i) * npm1finv, tapow);
