...
void main() {
	...
	if(mod(int(gl_FragCoord.x) ,2) == frameMod){
		vec4 ids2 = texture2DRect(lookUp, gl_TexCoord[0].xy + vec2(1.0,0.0));
	...
	//gl_FragColor = vec4(gl_FragCoord.x - 2.0 * floor( gl_FragCoord.x / 2.0));
