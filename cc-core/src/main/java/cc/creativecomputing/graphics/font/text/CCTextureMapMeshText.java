	...
	public void breakText() {
		...
		for(CCTextGridLinePart myPart:_myTextGrid.gridLines()) {
			for (int i = 0; i < myPart.charIndices().length; i++) {
				...
				final float myWidth = myPart.font().width(myCharIndex) * size();
				final float myHeight = myPart.font().height() * size();

				float myX = _myPosition.x + myPart.x(i);
				float myY = _myPosition.y + myPart.y() + myPart.font().ascent();
