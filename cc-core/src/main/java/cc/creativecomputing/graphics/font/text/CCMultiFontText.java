...
public class CCMultiFontText {
	...
	public static class CCTextPart{
		 ...
		 */
		public void font(final CCFont<?> theFont, final float theSize) {
			font(theFont);
			if (theFont != null)
				size(theSize);
		}
	...
	public class CCTextGridLinePart{
		...
		private CCTextGridLinePart(
			...
			_myX[0] = theX;
			final float mySpace = (float)(_myWidth - _myTextPart.font().width(theTextBuffer, theStart, theEnd)* _myTextPart.size())/(theEnd - theStart - 1);
			int myLastIndex = -1;
		...
		private void drawGrid(CCGraphics g) {
			for(int i = 0; i < _myX.length;i++) {
				...
				float myY = _myY + _myPosition.y + _myTextPart.ascent();
