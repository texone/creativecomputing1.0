...
//		
//		float myBlockWidth = theText.blockWidth();
//		float myBlockHeight = theText.blockHeight();
//
//		float myRunningX = 0;
//		float myCurrentX = 0;
//		float myCurrentY = 0;
...
//			int index = 0;
//			
//			final float mySpaceWidth = myPart.font().spaceWidth() * myPart.size();
...
//					// boundary of a word
//					float wordWidth = myPart.font().width(myTextBuffer, myWordStart, index) * myPart.size();
//	
...
//			myMaxLeading = myPart.leading();//CCMath.max(myMaxLeading, );
			while (index < myTextBuffer.length){
				...
				float myCharWidth = myPart.font().width(myChar) * myPart.size();
