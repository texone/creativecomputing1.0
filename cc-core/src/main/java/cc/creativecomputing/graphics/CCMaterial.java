...
public class CCMaterial {
	...
	public void draw(CCGraphics g){
		if(_myAmbient != null)g.gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GLLightingFunc.GL_AMBIENT, _myAmbient.array(), 0);
		if(_myDiffuse != null){
			_myDiffuse.alpha(_myAlpha);
			g.gl.glMaterialfv(GL.GL_FRONT, GLLightingFunc.GL_DIFFUSE, _myDiffuse.array(), 0);
		}
		if(_mySpecular != null)g.gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GLLightingFunc.GL_SPECULAR, _mySpecular.array(), 0);
		if(_myEmission != null)g.gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GLLightingFunc.GL_EMISSION, _myEmission.array(), 0);
		if(_myShininess != -1)g.gl.glMateriali(GL.GL_FRONT_AND_BACK, GLLightingFunc.GL_SHININESS, _myShininess);
	}
}
