...
public class CCWaterRippleFilter extends CCImageFilterFIR {
	...
	@CCControl(name = "damping", min = 0.0f, max = 1f)
	private float _cDamping = 1f;
	
	@CCControl(name = "wave speed", min = 0.0f, max = 6f)
	private float _cSpeed = 1f;
	
	private CCGLSLShader _myShader0;
	
	public CCWaterRippleFilter (CCGraphics theGraphics, CCTexture2D theInput) {
		super (theGraphics, theInput, 3);
		_myShader0 = new CCGLSLShader(CCIOUtil.classPath(this, "shader/water_propagate_vp.glsl"), CCIOUtil.classPath(this, "shader/water_propagate_fp.glsl"));
		_myShader0.load();
	}
