...
//	}	
	...
	public FloatBuffer getData(final int theAttachment){
		return getData(theAttachment, 0, 0, _myWidth, _myHeight);
	}
	
	public FloatBuffer getData(final int theAttachment, final int theX, final int theY, final int theWidth, final int theHeight) {
		FloatBuffer myResult = FloatBuffer.allocate(theWidth * theHeight * _myAttributes.textureAttributes(theAttachment).format().numberOfChannels);
...
//		if(myError != null)throw new CCFrameBufferObjectException(myError);
	...
	public ByteBuffer getBytes(final int theX, final int theY, final int theWidth, final int theHeight){
		return getBytes(0, theX, theY, theWidth, theHeight);
	}
	
	public ByteBuffer getBytes(final int theAttachment){
		return getBytes(theAttachment, 0, 0, _myWidth, _myHeight);
	}
	
	public ByteBuffer getBytes(final int theAttachment, final int theX, final int theY, final int theWidth, final int theHeight) {
		GL2 gl = CCGraphics.currentGL();
		
		int size = GLBuffers.sizeof(
			gl, 
			new int[1], 
			_myAttributes.textureAttributes(theAttachment).format().glID, 
			_myAttributes.textureAttributes(theAttachment).pixelType().glID, 
			theWidth, 
			theHeight, 
			1, 
			false
		);
		ByteBuffer myResult = ByteBuffer.allocate(size);
		
		
		gl.glBindFramebuffer(GL.GL_FRAMEBUFFER, _myFrameBuffers[0]);
		gl.glReadBuffer(_myDrawBuffers[theAttachment]);
		gl.glReadPixels(theX, theY, theWidth, theHeight,_myAttachments[theAttachment].format().glID,GL.GL_UNSIGNED_BYTE,myResult);
		gl.glBindFramebuffer(GL.GL_FRAMEBUFFER, 0);
		myResult.rewind();
//		String myError = checkError(gl);
//		if(myError != null)throw new CCFrameBufferObjectException(myError);
