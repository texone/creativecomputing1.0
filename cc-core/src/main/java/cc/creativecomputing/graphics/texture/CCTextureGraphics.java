...
public class CCTextureGraphics extends CCTexture2D{
	
	private CCTexture _myTexture;
	private BufferedImage _myRenderImage;
	private Graphics2D _myGraphics2D;

	public CCTextureGraphics(final int theWidth, final int theHeight, final boolean theIsMipMap) {
		_myRenderImage = new BufferedImage(theWidth, theHeight, BufferedImage.TYPE_INT_ARGB);
		_myTexture = new CCTexture2D(CCTextureIO.newTextureData(_myRenderImage));
	...
	/**
	 * After drawing the changes have to be applied to the opengl texture. Use this method
	 * to apply the changes.
	 */
	public void update(){
		_myTexture.updateData(CCTextureIO.newTextureData(_myRenderImage));
	}
...
//	}
	...
	public boolean mustFlipVertically() {
		return _myTexture.mustFlipVertically();
	}

	@Override
	public int id() {
		return _myTexture.id();
	}
