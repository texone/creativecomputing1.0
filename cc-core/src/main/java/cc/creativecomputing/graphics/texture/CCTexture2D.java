...
public class CCTexture2D extends CCTexture{
	...
	public CCTexture2D(CCTextureTarget theTarget, int theTextureID) {
		super(theTarget, theTextureID);
	}
...
//	            	ptr.rewind();
	...
	public void updateData(CCTextureData theData) {
		if(
			theData.width() != _myWidth || 
			theData.height() != _myHeight) {
			data(theData);
		}else {
	...
	public CCPixelMap getPixels(int theX, int theY, int theWidth, int theHeight) {
		FloatBuffer myBuffer = FloatBuffer.allocate(4 * _myWidth * _myHeight);
		GL2 gl = CCGraphics.currentGL();
		bind();
		gl.glReadPixels(theX, theY, theWidth, theHeight, CCPixelFormat.RGBA.glID, CCPixelType.FLOAT.glID, myBuffer);
		myBuffer.rewind();
		
		return new CCPixelMap(myBuffer, theWidth, theHeight, _myMustFlipVertically);
	}
