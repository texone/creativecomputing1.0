 ...
 */
public class CCConvolutionFilter extends CCTextureInputFilter {
	 ...
	 */
	public CCConvolutionFilter(float[] matrix) {
	 ...
	 */
	public CCConvolutionFilter(int rows, int cols, float[] matrix) {
	 ...
	 */
	public CCConvolutionFilter(Kernel kernel) {
		this.kernel = kernel;	
	}
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
        ...
        if ( premultiplyAlpha )
			CCImageMath.premultiply( inPixels, 0, inPixels.length );
		convolve(kernel, inPixels, outPixels, width, height, alpha, edgeAction);
        if ( premultiplyAlpha )
			CCImageMath.unpremultiply( outPixels, 0, outPixels.length );
	...
	public static void convolveHV(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, boolean alpha, int edgeAction) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				int ia = alpha ? CCPixelUtils.clamp((int)(a+0.5)) : 0xff;
				int ir = CCPixelUtils.clamp((int)(r+0.5));
				int ig = CCPixelUtils.clamp((int)(g+0.5));
				int ib = CCPixelUtils.clamp((int)(b+0.5));
