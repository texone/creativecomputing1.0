...
public class CCPixelStorageModes {
	 ...
	 */
	public void unpackStorage() {
		GL gl = CCGraphics.currentGL();

		gl.glPixelStorei(GL2.GL_UNPACK_SWAP_BYTES, _mySwapBytes ? 1 : 0);
		gl.glPixelStorei(GL2.GL_UNPACK_LSB_FIRST, _myLSBFirst ? 1 : 0);
		
		gl.glPixelStorei(GL2.GL_UNPACK_ROW_LENGTH, _myRowLength);
		gl.glPixelStorei(GL.GL_UNPACK_ALIGNMENT, _myAlignment);
		gl.glPixelStorei(GL2.GL_UNPACK_IMAGE_HEIGHT, _myImageHeight);
		
		gl.glPixelStorei(GL2.GL_UNPACK_SKIP_PIXELS, _mySkipPixels);
		gl.glPixelStorei(GL2.GL_UNPACK_SKIP_ROWS, _mySkipRows);
		gl.glPixelStorei(GL2.GL_UNPACK_SKIP_IMAGES, _mySkipImages);
	}
	 ...
	 * @see #defaultUnpackStorage()
	 */
	public void packStorage() {
		GL gl = CCGraphics.currentGL();

		gl.glPixelStorei(GL2.GL_PACK_SWAP_BYTES, _mySwapBytes ? 1 : 0);
		gl.glPixelStorei(GL2.GL_PACK_LSB_FIRST, _myLSBFirst ? 1 : 0);
		
		gl.glPixelStorei(GL2.GL_PACK_ROW_LENGTH, _myRowLength);
		gl.glPixelStorei(GL.GL_PACK_ALIGNMENT, _myAlignment);
		gl.glPixelStorei(GL2.GL_PACK_IMAGE_HEIGHT, _myImageHeight);
		
		gl.glPixelStorei(GL2.GL_PACK_SKIP_PIXELS, _mySkipPixels);
		gl.glPixelStorei(GL2.GL_PACK_SKIP_ROWS, _mySkipRows);
		gl.glPixelStorei(GL2.GL_PACK_SKIP_IMAGES, _mySkipImages);
	}
	 ...
	 * @see #unpackStorage()
	 */
	public void defaultUnpackStorage() {
		GL gl = CCGraphics.currentGL();
