...
public class CCTesselatorMesh implements GLUtessellatorCallback{
	...
	protected List<CCVector3f> _myVertices = new ArrayList<CCVector3f>();
	protected List<CCVector2f> _myTextureCoords = new ArrayList<CCVector2f>();
	protected List<CCVector3f> _myNormals = new ArrayList<CCVector3f>();
...
//	      throw new RuntimeException("Tessellation Error: " + estring);
	...
	public void vertex(final Object theVertexData) {
		if (theVertexData instanceof double[]) {
			...
			if (d.length >= 3) {
				_myVertices.add(new CCVector3f((float)d[0],(float)d[1],(float)d[2]));
			}
			if (d.length == 5) {
				_myTextureCoords.add(new CCVector2f((float)d[3],(float)d[4]));
			}
			if (d.length == 6) {
				_myNormals.add(new CCVector3f((float)d[3],(float)d[4],(float)d[5]));
			}
			if (d.length == 8) {
				_myNormals.add(new CCVector3f((float)d[3],(float)d[4],(float)d[5]));
				_myTextureCoords.add(new CCVector2f((float)d[6],(float)d[7]));
			}
		} else {
	...
	public void vertex(
		...
		final float theTX, final float theTY	
	){
	...
	public void vertex(
		final float theX, final float theY, final float theZ,
		final float theNX, final float theNY, final float theNZ	
	){
	...
	public void vertex(final CCVector3f theVertex,final CCVector3f theNormal){
		vertex(
			theVertex.x, theVertex.y, theVertex.z, 
	...
	public void vertex(final CCVector3f theVertex,final CCVector3f theNormal,final CCVector2f theTextureCoords){
		vertex(
			theVertex.x, theVertex.y, theVertex.z, 
			theNormal.x, theNormal.y,theNormal.z, 
			theTextureCoords.x, theTextureCoords.y
