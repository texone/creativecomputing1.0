...
/**
 * A fairly direct port of Brian Paul's tile rendering library, found at <a href =
 * "http://www.mesa3d.org/brianp/TR.html"> http://www.mesa3d.org/brianp/TR.html </a> . I've java-fied it, but the
 * functionality is the same.
 * 
 * Original code Copyright (C) 1997-2005 Brian Paul. Licensed under BSD-compatible terms with permission of the author.
...
public class TileRenderer {
	...
	private boolean _myUsePerspective;
	...
	private float _myNear;
	private float _myFar;
	...
	/**
	 * Sets the size of the tiles to use in rendering. The actual effective size of the tile depends on the border size,
	 * ie ( width - 2*border ) * ( height - 2 * border )
	 * 
	 * @param width The width of the tiles. Must not be larger than the GL context
	 * @param height The height of the tiles. Must not be larger than the GL context
	 * @param border The width of the borders on each tile. This is needed to avoid artifacts when rendering lines or
	 *        points with thickness > 1.
	 */
	...
	/**
	 * Specify a buffer the tiles to be copied to. This is not necessary for the creation of the final image, but useful
	 * if you want to inspect each tile in turn.
	 * 
	 * @param format Interpreted as in glReadPixels
	 * @param type Interpreted as in glReadPixels
	 * @param image The buffer itself. Must be large enough to contain a tile, minus any borders
	 */
	 ...
	 * 
	 * @param order The row traversal order, must be eitherTR_TOP_TO_BOTTOM or TR_BOTTOM_TO_TOP
	 */
	 ...
	 * 
	 * @param theLeft As in glOrtho
	 * @param theRight As in glOrtho
	 * @param theBottom As in glOrtho
	 * @param theTop As in glOrtho
	 * @param theZNear As in glOrtho
	 * @param theZFar As in glOrtho
	 */
	public void ortho(
		final float theLeft, final float theRight, 
		final float theBottom, final float theTop, 
		final float theZNear, final float theZFar
	) {
		_myUsePerspective = false;
		_myLeft = theLeft;
		_myRight = theRight;
		_myBottom = theBottom;
		_myTop = theTop;
	 ...
	 * 
	 * @param theLeft As in glFrustrum
	 * @param theRight As in glFrustrum
	 * @param theBottom As in glFrustrum
	 * @param theTop As in glFrustrum
	 * @param theZNear As in glFrustrum
	 * @param theZFar As in glFrustrum
	 */
	public void frustum(
		final float theLeft, final float theRight, 
		final float theBottom, final float theTop, 
	 ...
	 * 
	 * @param fovy As in gluPerspective
	 * @param aspect As in gluPerspective
	 * @param zNear As in gluPerspective
	 * @param zFar As in gluPerspective
	 */
	public void perspective(final float fovy, float aspect, float zNear, float zFar) {
		float xmin, xmax, ymin, ymax;
		ymax = zNear * CCMath.tan(fovy * CCMath.PI / 360);
		ymin = -ymax;
		xmin = ymin * aspect;
		xmax = ymax * aspect;
	 ...
	 */
	public void beginTile(CCGraphics g) {
		if (currentTile <= 0) {
			setup();
			/*
			 * Save user's viewport, will be restored after last tile rendered
			 */
			g.gl.glGetIntegerv(GL.GL_VIEWPORT, viewportSave, 0);
		}
		...
		CCViewport myViewport= new CCViewport(0, 0, tw, th);

		g.camera().viewport(myViewport);

		/* save current matrix mode */
		CCMatrixMode matrixMode = g.matrixMode();
		g.matrixMode(CCMatrixMode.PROJECTION);
		g.loadIdentity();
		
		perspective(g.camera().fov(), g.camera().aspect(), g.camera().near(), g.camera().far());

		/* compute projection parameters */
		double l = _myLeft + (_myRight - _myLeft) * (currentColumn * tileSizeNB.width - border) / imageSize.width;
		double r = l + (_myRight - _myLeft) * tw / imageSize.width;
		double b = _myBottom + (_myTop - _myBottom) * (currentRow * tileSizeNB.height - border) / imageSize.height;
		double t = b + (_myTop - _myBottom) * th / imageSize.height;

		if (_myUsePerspective) {
			g.gl.glFrustum(l, r, b, t, _myNear, _myFar);
		} else {
			g.gl.glOrtho(l, r, b, t, _myNear, _myFar);
		}
//
//		/* restore user's matrix mode */
		g.matrixMode(matrixMode);
	}
	...
	public boolean endTile(GL gl) {
		int[] prevRowLength = new int[1], prevSkipRows = new int[1], prevSkipPixels = new int[1], prevAlignment = new int[1];
	 ...
	 * 
	 * @param x As in glRasterPos3f
	 * @param y As in glRasterPos3f
	 * @param z As in glRasterPos3f
	 * @param gl The gl context
	 * @param glu A GLU object
	 */
	public void trRasterPos3f(float x, float y, float z, GL2 gl, GLU glu) {
		if (currentTile < 0) {
