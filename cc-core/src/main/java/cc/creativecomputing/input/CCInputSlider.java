...
public class CCInputSlider extends CCInput {
	...
	void update(final float theDeltaTime) {
		...
		if (Math.abs(_myActualValue) < _myComponent.getDeadZone() + _myTolerance) {
			...
			int mySign = CCMath.sign(myValue);
			_myActualValue = CCMath.pow(_myComponent.getPollData(),_myPower) * _myMultiplier;
			_myActualValue *= CCMath.sign(_myActualValue) == mySign ? 1 : -1;
		}
