...
public class CCXMLObjectSerializer {
	 ...
	 * @return
	 */
	private <Type> Class<Type> checkClass(CCXMLElement theElement, Class<Type> theClass){
	...
	private void getFields(final Class<?> theClass, final List<Field> theFields) {
		...
		for (Field myField : theClass.getDeclaredFields()) {
			...
			CCProperty myControl = myField.getAnnotation(CCProperty.class);
	 ...
	 * @return
	 */
	private boolean readAttribute(CCXMLElement theElement, CCProperty theAttribute) {
	...
	private void setPrimitiveField(CCXMLElement theElement, CCProperty theAttribute, Class theClass, Field theField, Object theObject) throws IllegalArgumentException, IllegalAccessException {
	...
	private void setPrimitiveMethod(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Method myMethod, Object myObject) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
	...
	private void setArrayField(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Field myField, Object myObject) throws ArrayIndexOutOfBoundsException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, SecurityException, NoSuchMethodException {
		...
		if(myAttribute.node()) {
			List<CCXMLElement> myArrayElements = theElement.child(myAttribute.name()).children();

			Object myArrayObject = Array.newInstance(myArrayType, myArrayElements.size());
			
			int myIndex = 0;
			
			for(CCXMLElement myArrayElement:myArrayElements) {
	...
	private void setCollectionField(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Field myField, Object myObject) throws ArrayIndexOutOfBoundsException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, SecurityException, NoSuchMethodException {
	...
	private void setEnumField(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Field myField, Object myObject) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
	...
	private void setEnumMethod(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Method myMethod, Object myObject) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
	...
	private void setStringField(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Field myField, Object myObject) throws IllegalArgumentException, IllegalAccessException {
	...
	private void setStringMethod(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Method myMethod, Object myObject) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
	...
	private void setObjectField(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Field myField, Object myObject) throws IllegalArgumentException, IllegalAccessException {
		CCXMLElement myChild = theElement.child(myAttribute.name());
	...
	private void setObjectMethod(CCXMLElement theElement, CCProperty myAttribute, Class myClass, Method myMethod, Object myObject) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		CCXMLElement myChild = theElement.child(myAttribute.name());
	 ...
	 * @return
	 */
	public <Type> Type toObject(CCXMLElement theElement, Class<Type> theClass){
		theClass = checkClass(theElement, theClass);
		
		CCPropertyObject myXLNode = theClass.getAnnotation(CCPropertyObject.class);
		if(myXLNode == null) {
		...
		for (Field myField : myFields) {
			...
			CCProperty myAttribute = myField.getAnnotation(CCProperty.class);
		...
		for (Method myMethod : theClass.getDeclaredMethods()) {
			...
			CCProperty myAttribute = myMethod.getAnnotation(CCProperty.class);
	 ...
	 * @return
	 */
	public <Type> List<Type> getObjects(CCXMLElement theElement, Class<Type> theClass){
		List<Type> myResult = new ArrayList<Type>();
		
		CCPropertyObject myXMLNode = theClass.getAnnotation(CCPropertyObject.class);
		if(myXMLNode == null)return myResult;
		
		String myNodeName = myXMLNode.name();
		
		for(CCXMLElement myChild:theElement) {
	...
	public Object toObject(CCXMLElement theElement) {
		return toObject(theElement,null);
	}
	
	
	private void addChildToXml(CCXMLElement theElement, Object theObject) {
		...
		for (Field myField : myFields) {
			...
			try {
				if(myClass.isPrimitive()) {
					...
					CCPropertyObject myXMLNode = myObject.getClass().getAnnotation(CCPropertyObject.class);
					
					if(myXMLNode == null)continue;
					
					CCXMLElement myFieldXML = new CCXMLElement(myAttribute.name());
	 ...
	 * save an object to xml it must be marked with {@linkplain CCPropertyObject} annotation.
	 * @param theObject the object to add to the xml
	 */
	public void addChild(CCXMLElement theElement, Object theObject) {
		CCPropertyObject myXMLNode = theObject.getClass().getAnnotation(CCPropertyObject.class);
		if(myXMLNode == null)return;
		
		CCXMLElement myElement = new CCXMLElement(myXMLNode.name());
