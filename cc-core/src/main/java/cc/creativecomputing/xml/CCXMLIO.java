...
//					throw new RuntimeException("Illegal use of &. Use &amp; entity instead. Line:"+line);
	 ...
	 * 
	 * @param <XMLTool>
	 * @param theDocumentUrl url from where the Element has to be loaded
	...
	public <XMLTool extends CCAbstractXMLTool> void parseXML(
		final String theDocumentUrl, 
		final XMLTool theXMLTool, 
		final CCIXMLFeedbackHandler<XMLTool> theFeedbackHandler,
		final boolean theIgnoreLineFeed
	){
		if (theDocumentUrl.startsWith("<?xml")){
			readXML(new StringReader(theDocumentUrl), theXMLTool, theFeedbackHandler,theIgnoreLineFeed);
		}else{
			loadElement(CCIOUtil.openStream(theDocumentUrl), theXMLTool, theFeedbackHandler,theIgnoreLineFeed);
		}
	}
	...
	public <XMLTool extends CCAbstractXMLTool> void loadElement(
		...
		parseXML(theDocumentUrl, theXMLTool,theFeedbackHandler,true);
	}
	...
	public <XMLTool extends CCAbstractXMLTool> XMLTool loadElement(
		...
		return loadElementNow(documentUrl, theXMLTool,theIgnoreLineFeed);
	}
	...
	public <XMLTool extends CCAbstractXMLTool> XMLTool loadElement(
		...
		return loadElementNow(documentUrl, theXMLTool,true);
	}
	...
	public <XMLTool extends CCAbstractXMLTool> XMLTool loadElementNow(
		...
		try{
			InputStream test = CCIOUtil.openStream(documentUrl);
	 ...
	 */
	public static CCXMLElement createXMLElement(final String theDocumentUrl, final boolean theIgnoreLineFeed){
		if(_myInstance == null){
			_myInstance = new CCXMLIO();
		}
		CCXMLTree myTree = _myInstance.loadElement(theDocumentUrl, new CCXMLTree(),theIgnoreLineFeed);
		if(myTree != null) {
	...
	public static CCXMLElement createXMLElement(final String theDocumentUrl){
		return createXMLElement(theDocumentUrl, true);
	}
	 ...
	 * @param theFilename path to save the XMLElement as XML File 
	 * @param theEncoding used for the XML File 
	 */
	public static void saveXMLElement(final CCXMLElement theXMLElement, String theFilename, String theEncoding){
		try{
			PrintStream myWriter = new PrintStream(CCIOUtil.createOutputStream(theFilename), true, theEncoding);
	 ...
	 * @param theFilename path to save the XMLElement as XML File 
	 */
	public static void saveXMLElement(final CCXMLElement theXMLElement, String theFilename){
		saveXMLElement(theXMLElement, theFilename, "ISO-8859-1");
	}
