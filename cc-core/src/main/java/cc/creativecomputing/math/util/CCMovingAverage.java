...
public abstract class CCMovingAverage <Type>{
	...
	public static CCMovingAverage<CCVector3f> vector3fAverage(final float theWeight){
		return new CCVector3fAverage(theWeight);
	}
	
	private static class CCVector3fAverage extends CCMovingAverage<CCVector3f>{
		...
		@Override
		public CCVector3f update(CCVector3f theValue) {
			if(_myValue == null) {
	...
	public static CCMovingAverage<CCVector2f> vector2fAverage(final float theWeight){
		return new CCVector2fAverage(theWeight);
	}
	
	private static class CCVector2fAverage extends CCMovingAverage<CCVector2f>{
		...
		@Override
		public CCVector2f update(CCVector2f theValue) {
			if(_myValue == null) {
				_myValue = theValue.clone();
				return _myValue;
			}
