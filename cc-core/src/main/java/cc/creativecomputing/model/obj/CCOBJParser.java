...
import cc.creativecomputing.model.CCObjectGroup;
...
public class CCOBJParser extends CCAbstractOBJParser {
	...
	public CCOBJParser(final String theFileName, final CCModel theModel)
			throws CCOBJParsingException {
	...
	private void readNormal() throws CCOBJParsingException {
		_myModel.normals().add(new CCVector3f(getFloat(), getFloat(), getFloat()).normalize(0.2f));
	...
	private void readTexture() throws CCOBJParsingException {
		_myModel.textureCoords().add(new CCVector2f(getFloat(), getFloat()));
	...
	private void loadMaterialFile() throws CCOBJParsingException {
		...
		try {
			new CCMTLParser(_mySourceFolder + s, _myModel.materialMap())
					.readFile();
