...
import cc.creativecomputing.model.material.CCTextureProperty;
...
class CCMTLParser extends CCAbstractOBJParser{
	...
	private final String _mySourceFolder;
	private final HashMap<String,CCModelMaterial> _myMaterialMap;
	private CCModelMaterial _myCurrentMaterial;
	
	CCMTLParser(final String theFileName, final HashMap<String,CCModelMaterial> theMaterialMap)throws CCOBJParsingException{
	...
	private void readName() throws CCOBJParsingException {
		...
		if (ttype == TT_WORD) {
			...
			_myCurrentMaterial = new CCModelMaterial();
...
//	}
	...
	private void readMapKd()throws CCOBJParsingException {
		...
		getToken();
		String texname = sval;
		_myCurrentMaterial.addProperty(new CCTextureProperty(new CCTexture2D(CCTextureIO.newTextureData(_mySourceFolder+File.separator+texname))));
		lowerCaseMode(true);
	}
