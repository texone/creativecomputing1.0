...
uniform samplerRECT staticPositions;
...
void main (
	...
	float3 staticPosition = texRECT (staticPositions, texID);
	newPosition = float4(lerp(position + deltaTime * velocity, staticPosition, staticPositionBlend),1); 
}
