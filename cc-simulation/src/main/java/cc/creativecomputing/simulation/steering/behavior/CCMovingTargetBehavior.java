...
public class CCMovingTargetBehavior extends CCForce{
	...
	public CCVector3f predictedTarget(){
	...
	public boolean apply(final CCParticle theAgent, final CCVector3f theForce, float theDeltaTime){
		float d = theAgent.position.distance(target.position);

		float normalness = 1.0F - Math.abs(theAgent.forward.dot(target.forward));
		d *= 1.1F + normalness * 0.2F;
		
		targetBehavior._myTarget.set(target.velocity().clone());
		targetBehavior._myTarget.scale(d);
		targetBehavior._myTarget.add(target.position);
