...
import cc.creativecomputing.simulation.particles.forces.blend.CCGPUIDTextureBlendForce;
...
public class CCColladaModelParticlesDemo extends CCApp {
	...
	private class CCSkeletonParticles {
		...
		public CCSkeletonParticles(CCApp theApp, CCShaderBuffer theTargets, int theXRes, int theYRes) {
			...
			_myBlendTexture = new CCShaderBuffer(theXRes, theYRes);
			...
			myForces.add(_myBlendForce = new CCGPUIDTextureBlendForce(_myBlendTexture.attachment(0), _myTargetForce));
			...
			_myParticles = new CCGPUParticles(theApp.g, _myRenderer, myForces, myConstraints, theXRes, theYRes);
		...
		public void update(float theDeltaTime) {
			float myX1 = 0;
			float myX2 = _myBlendTexture.width();
			
			float myGradientHeight = _cGradientRange * _myBlendTexture.height();
			float myHeight = _myBlendTexture.height() + myGradientHeight;
			
			float myY1 = _cGradientBlend * myHeight - myHeight;
			float myY2 = myY1 + _myBlendTexture.height();
			float myY3 = myY2 + myGradientHeight;
			
			_myBlendTexture.beginDraw();
			g.clearColor(_cBlend);
			g.clear();
			...
			g.vertex(myX2,myY3);
			g.endShape();
			_myBlendTexture.endDraw();
			_myBlendForce.pow(_cBlendPow);
			
			_myForceField.strength(_cNoise);
			_myTargetForce.strength(_cTarget);
			_myTargetForce.maxForce(_cTargetMaxForce);
			...
			_myFloorConstraint.resilience(_cFloorResilience);
			_myFloorConstraint.friction(_cFloorFriction);
			_myFloorConstraint.minimalVelocity(_cFloorMinVel);
...
//			g.blend(CCBlendMode.ADD);
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(CCColladaModelParticlesDemo.class);
