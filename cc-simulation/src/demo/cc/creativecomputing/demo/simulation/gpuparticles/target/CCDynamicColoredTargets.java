...
public class CCDynamicColoredTargets extends CCApp{
	...
	public void setup() {
		...
		myForces.add(new CCGPUViscousDrag(0.1f));
		
		_myTimeBlendForce = new CCGPUTimeForceBlend(0,4, _myForceField = new CCGPUForceField(0.01f, 1f, new CCVector3f()), _myTargetForce);
		_myTimeBlendForce.blend(0.005f, 1f);
		_myTimeBlendForce.power(6);
		myForces.add(_myTimeBlendForce);

		_myParticles = new CCGPUParticles(g, new CCGPUIndexedParticleRenderer(),myForces, new ArrayList<CCGPUConstraint>(), 700, 700);
		...
		_myInitValueShader = new CCCGShader(
			null, 
			CCIOUtil.classPath(CCGPUParticles.class, "shader/initvalue.fp")
		);
