...
public class JSMinim implements MinimServiceProvider
{
  ...
  public Mixer getOutputMixer()
  {
			...
			boolean buffered)
	{
		...
		{
			throw new JSException("The extension " + ext + " is not a recognized audio file type.");
		}
		...
		{
			throw new JSException( "Couldn't invoke the sketchPath method", ex );
		}
	...
	private Map<String, Object> getID3Tags(String filename)
	{
		...
		try
		{
			...
			if (baseFileFormat instanceof TAudioFileFormat)
			{
				...
				if (props.size() == 0)
				{
					throw new JSException("No file properties available for " + filename + ".");
				}
				else
				{
		...
		{
			throw new JSException("Couldn't get the file format for " + filename,e);
		}
		catch (IOException e)
		{
			throw new JSException("Couldn't access " + filename + "", e);
		}
		catch( Exception e )
		{
			throw new JSException("Error invoking createInput on the file loader object", e);
		}
	...
	public AudioSample getAudioSample(String filename, int bufferSize)
	{
		...
		if (ais != null)
		{
			...
			{
				throw new JSException("Couldn't acquire an output.");
			}
		}
		return null;
	}
  ...
  private JSAudioSample getAudioSampleImp(FloatSampleBuffer samples, AudioFormat format, int bufferSize)
  {
	...
	public AudioRecording getAudioRecordingClip(String filename)
	{
		...
		if (ais != null)
		{
			...
			if (AudioSystem.isLineSupported(info))
			{
				...
				{
					throw new JSException("Error obtaining Javasound Clip", e);
				}
				Map<String, Object> props = getID3Tags(filename);
				long lengthInMillis = -1;
				if (props.containsKey("duration"))
			...
			{
				throw new JSException("File format not supported.");
			}
		}
		if (meta == null)
		{
	...
	private FloatSampleBuffer loadFloatAudio(AudioInputStream ais, int toRead)
	{
		...
		{
			throw new JSException("Error loading file into memory", ioe);
		}
   ...
   */
	AudioInputStream getAudioInputStream(String filename)
	{
		...
		if (filename.startsWith("http"))
		{
			...
			{
				throw new JSException("Bad URL", e);
			}
			catch (UnsupportedAudioFileException e)
			{
				throw new JSException("URL is in an unsupported audio file format ", e);
			}
			catch (IOException e)
			{
				throw new JSException("Error reading the URL ", e);
			}
		}
		else
		{
			...
			{
				throw new JSException("IOException", ioe);
			}
			catch (UnsupportedAudioFileException uafe)
			{
				throw new JSException("Unsupported Audio File", uafe);
			}
			catch( Exception e )
			{
				throw new JSException( "Error invoking createInput on the file loader object", e );
	...
	SourceDataLine getSourceDataLine(AudioFormat format, int bufferSize)
	{
        ...
        else
        {
          line = (SourceDataLine)outputMixer.getLine(info);
			...
			{
				throw new JSException("Couldn't open the line", e);
			}
		}
		throw new JSException("Unable to return a SourceDataLine: unsupported format - " + format.toString());
	}
	...
	TargetDataLine getTargetDataLine(AudioFormat format, int bufferSize)
	{
        ...
        else
        {
          line = (TargetDataLine)inputMixer.getLine(info);
			...
			{
				throw new JSException("Error acquiring TargetDataLine", e);
			}
		}
		else
		{
			throw new JSException("Unable to return a TargetDataLine: unsupported format - " + format.toString());
		}
		return line;
	}
