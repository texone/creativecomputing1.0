...
final class JSBufferedSampleRecorder implements SampleRecorder
{
  ...
  public AudioRecordingStream save()
  {
    if ( isRecording() )
    {
      throw new JSException("You must stop recording before you can write to a file.");
    }
    else
    {
      ...
      if (AudioSystem.isFileTypeSupported(type, ais))
      {
        ...
        {
          throw new JSException("AudioRecorder.save: Error attempting to save buffer to "
              + name + "\n" + e.getMessage());
        }
        if (out.length() == 0)
        {
      ...
      {
        throw new JSException("AudioRecorder.save: Can't write " + type.toString()
