...
final class JSStreamingSampleRecorder implements SampleRecorder
{
                         ...
                         int bufferSize)
  {
    ...
    {
    	throw new JSException("Error obtaining new output stream", e);
    }
    catch (IllegalArgumentException badarg)
    {
    	throw new JSException("Error obtaining new output stream for " + fileName + " with type " 
          + type.toString() + " format " + format.toString() 
          + " and bufferSize " + bufferSize + ".\n" 
          + "The reason is ", badarg);
    }
